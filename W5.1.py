...
def increase_prices(fruit_prices: dict) -> None:
    ...
    Return:
    None - Do not return anything - modify inplace
    '''
...
def dict_from_string(string: str, key_type, value_type):
    ...
    Also convert the types of key and values to the given types.
    ...
    '''
    return {key_type(k): value_type(v) for k, v in 
            (line.strip().split(',') for line in string.strip().split('\n'))}

def dict_to_string(D: dict) -> str:
    '''
    Convert the given dictionary back to the string format produced by `dict_from_string`. 
    Again, do not use loops or conditionals, use comprehensions.
    '''
